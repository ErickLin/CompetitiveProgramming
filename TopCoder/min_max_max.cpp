...
class MinMaxMax {
    public:
        ll findMin(vi a, vi b, vi w, vi v) {
            int m = w.size(), n = v.size();
            vvll adj(n, vll(n)), d(n, vll(n));
            vector<pii> vp(n);
            forf(i, n) {
                fill(adj[i].begin(), adj[i].end(), 1ull << 40);
                fill(d[i].begin(), d[i].end(), 1ull << 60);
                vp[i] = mp(v[i], i);
            }
            ...
            forf(idx, n) {
                ...
                forf(i, n) {
                    forf(j, n) {
                        ...
                        d[i][j] = min(d[i][j], adj[i][j] * max(v[k], max(v[i], v[j])));
                    }
                }
            }
            ll ret = 0;
